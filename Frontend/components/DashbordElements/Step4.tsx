...
function step4() {
  ...
  const endGenerate = ()=>{
    setLoading(false);
    let now = new Date();
    setEndTime(now.toLocaleString());
    let during = Date.now() - stamp;
    console.log(during)
    if(during<60000) setDuringTime((during/1000).toFixed(1)+" secs");
    else setDuringTime(Math.floor(during/60000)+" mins and  "+ (during%60000===0?"":(((during%60000)/1000).toFixed(1)+"secs")));
  }
